...
import {
  ArrowRight, CheckCircle2, Zap, Shield, BarChart, Server, Mail, Activity,
  Send, Users, Filter, FlaskConical, BarChart3, TrendingUp, Globe, Lock,
  Cpu, Tag, ChevronDown, ChevronUp, CheckCircle, AlertTriangle, XCircle,
  Wifi, Code2, Building2, Briefcase,
} from "lucide-react";
...
const ALL_FEATURES = [
  {
    icon: Send, title: "Campaign Management",
    ...
    items: ["Multi-step campaign wizard","Schedule future sends","Pause & resume live campaigns","Graceful or immediate stop","Test email before sending","Cancel scheduled campaigns","Real-time progress tracking","Duplicate-send protection"],
  },
  {
    icon: Zap, title: "Email Template Editor",
    ...
    items: ["Drag-and-drop block builder","Raw HTML editor mode","Visual WYSIWYG mode","Desktop / tablet / mobile preview","Deliverability score panel","Spam word detection","Duplicate & export templates","Convert between editor modes"],
  },
  {
    icon: Users, title: "Subscriber Management",
    ...
    items: ["CSV bulk import with field mapping","Parallel chunk processing (15k/batch)","Background job with live progress","Duplicate detection & deduplication","Three-tier field system","Per-list export as CSV","Subscriber status management","Custom fields per subscriber"],
  },
  {
    icon: Filter, title: "Smart Segmentation",
    ...
    items: ["Subscriber status filter","List membership filter","Subscription date range","Geographic (country / city)","Engagement level (high / medium / low)","Email domain filter","Industry & company size","Live subscriber count preview"],
  },
  {
    icon: Activity, title: "Automation Workflows",
    ...
    items: ["Welcome sequence trigger","Birthday & re-engagement triggers","Abandoned cart & post-purchase","Conditional branch logic","A/B split steps","Webhook action steps","Goal tracking & exit conditions","Quiet hours & timezone support"],
  },
  {
    ...
    items: ["Subject line A/B test","Sender name / email A/B test","Configurable split percentage","Open rate or click rate criteria","Configurable test duration","Auto winner declaration","Auto-send winner to remainder","Statistical significance indicator"],
  },
  {
    ...
    items: ["Open & click rate tracking","Bounce & unsubscribe rates","Spam complaint rate","Top clicked links","Campaign progress tracking","Cross-campaign dashboard","Export per event type (CSV)","Date-range filtering (1–365 days)"],
  },
  {
    icon: TrendingUp, title: "Deliverability & Suppression",
    ...
    items: ["Global & list-specific suppressions","Bounce auto-suppression via webhook","Complaint auto-suppression","Bulk suppression import / export","List-Unsubscribe header (one-click)","Rate limiting per provider","Circuit breaker pattern","Dead letter queue (DLQ)"],
  },
  {
    icon: Globe, title: "Domain & DNS",
    desc: "Add custom sending domains with guided SPF, DKIM, and DMARC setup. Multiple domains for different brands or senders.",
    items: ["Custom sending domains","DNS verification flow","SPF record guidance","DKIM record guidance","DMARC record guidance","Multiple domain support","Verification status tracking","Re-verify on DNS change"],
  },
  {
    icon: Tag, title: "Personalisation",
    ...
    items: ["{{variable}} template syntax","Three-tier field mapping","Fallback value per field","Subject line personalisation","Jinja2 loops & conditionals","Nested object field access","Unsubscribe URL injection","System fields (date, year)"],
  },
  {
    ...
    items: ["JWT Bearer token auth","bcrypt password hashing","Security headers middleware","CORS configuration","HTTPS enforcement (HSTS)","Full audit trail","IP address logging","Password change flow"],
  },
  {
    icon: Cpu, title: "Infrastructure",
    ...
    items: ["Celery background workers","Dedicated queue per feature","Redis worker-level caching","Campaign content snapshot","Resource manager (CPU / memory)","Exponential retry backoff","Celery Beat scheduled tasks","MongoDB connection pooling"],
  },
];
  ...
  return (
    <Card className="hover:shadow-xl transition-shadow h-full group">
      <CardContent className="p-8 flex flex-col h-full">
        ...
        <ul className="space-y-2 mb-4">
          {preview.map((item) => (
            <li key={item} className="flex items-start gap-2 text-sm text-muted-foreground">
              <CheckCircle2 className="w-4 h-4 text-primary/60 flex-shrink-0 mt-0.5" />{item}
            </li>
          ))}
        ...
        {rest.length > 0 && (
          <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-1 text-xs font-semibold text-primary hover:underline self-start mt-auto">
            {expanded ? <><ChevronUp className="w-3.5 h-3.5" /> Show less</> : <><ChevronDown className="w-3.5 h-3.5" /> +{rest.length} more</>}
          </button>
        )}
      </CardContent>
    </Card>
...
const fadeUp = { initial: { opacity: 0, y: 24 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.55, ease: "easeOut" } };
...
export function Landing() {
  ...
  const deliveryPlans = [
    { name: "Starter Delivery", price: "$29/mo", volume: "100,000 emails/month", infra: "Shared IP pool", routing: "Basic routing" },
    { name: "Growth Delivery",  price: "$79/mo", volume: "500,000 emails/month", infra: "Optimized routing", routing: "Better inbox placement" },
    { name: "Scale Delivery",   price: "$149/mo", volume: "1,000,000 emails/month", infra: "Priority queue", routing: "Warmup system included" },
    { name: "Dedicated IP",     price: "$299+/mo", volume: "Custom email volume", infra: "Dedicated IP", routing: "Reputation management + scaling" },
  ];
    ...
    <PublicLayout>
      ...
      <section className="relative pt-24 pb-32 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 text-center">
          <motion.div {...fadeUp} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-8 border border-primary/20">
          ...
          <motion.p {...fadeUp} transition={{ duration: 0.55, delay: 0.1 }}
            className="text-lg font-semibold text-muted-foreground mb-3"
          >
            Self-hosted. Your server. Your control.
          </motion.p>

          <motion.p {...fadeUp} transition={{ duration: 0.55, delay: 0.15 }}
            className="text-lg text-muted-foreground max-w-2xl mx-auto mb-10"
          >
            Install on your own server and send using your SMTP — or upgrade to our optimized managed delivery anytime.
          </motion.p>

          <motion.div {...fadeUp} transition={{ duration: 0.55, delay: 0.2 }} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
            <Link href="#pricing">
      ...
      <section id="how" className="py-16 bg-muted/20">
        <div className="max-w-5xl mx-auto px-4">
          <motion.h2 {...fadeUp} className="text-center text-2xl font-bold mb-10 text-muted-foreground">
            Up and running in 5 steps
          </motion.h2>
          <motion.div {...stagger} className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <motion.div key={s.step} {...fadeUp}>
                <Card className="text-center hover:border-primary/40 transition-colors">
                  <CardContent className="p-5">
                    <p className="text-2xl font-extrabold text-primary/30 mb-1">{s.step}</p>
      ...
      <section className="py-20">
        <div className="max-w-5xl mx-auto px-4">
          <motion.div {...fadeUp} className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-3">Your infrastructure or ours — you choose</h2>
            <p className="text-muted-foreground text-lg">Start self-hosted, upgrade to managed delivery as you scale.</p>
          </motion.div>
                ...
                <CardContent className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Bring your own SMTP</p>
                    </div>
                  </div>
                  <ul className="space-y-3 text-sm text-muted-foreground mb-6">
                    {["Use your own SMTP (SES, SendGrid…)","Lowest cost at scale","Full provider control","Manual setup and management","Basic tracking & webhooks"].map(item => (
                      <li key={item} className="flex items-start gap-2">
                        <CheckCircle2 className="w-4 h-4 text-primary/40 flex-shrink-0 mt-0.5" />{item}
                      </li>
                    ))}
                  </ul>
                  <Link href="/register">
              ...
              <Card className="h-full border-primary shadow-xl relative overflow-hidden">
                ...
                <CardContent className="p-8">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      <h3 className="text-xl font-bold">Managed (Recommended)</h3>
                      <p className="text-xs text-primary font-semibold">Best for inbox placement</p>
                    </div>
                  </div>
                  <ul className="space-y-3 text-sm text-muted-foreground mb-6">
                    {["Better inbox placement rates","Optimized routing infrastructure","Dedicated IP provisioning","IP warmup management","Full reputation monitoring"].map(item => (
                      <li key={item} className="flex items-start gap-2">
                        <CheckCircle2 className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" />{item}
                      </li>
                    ))}
                  </ul>
                  <Button className="w-full gap-2">Get Managed Setup <ArrowRight className="w-4 h-4" /></Button>
                </CardContent>
              </Card>
            </motion.div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp} className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-lg text-muted-foreground">
              Every capability your email programme needs — campaigns, automation, A/B testing, segmentation, deliverability, and full infrastructure control.
            </p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {ALL_FEATURES.map((feature, i) => (
              <motion.div key={feature.title} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.04 }}>
      ...
      <section className="py-20">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <motion.div {...fadeUp} className="mb-12">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">Built for people who own their infrastructure</h2>
            <p className="text-muted-foreground text-lg">ZeniPost is the right fit if you are any of the following:</p>
          </motion.div>
            ...
            {[
              {
                ...
                points: ["Self-host on any VPS or cloud","Full API access","No per-email charges on your tier"],
              },
              {
                icon: Briefcase,
                title: "Agencies",
                ...
                points: ["Unlimited installs (Agency license)","White-label branding","Client usage permitted"],
              },
              {
                icon: Building2,
                title: "Businesses",
                ...
                points: ["Data stays on your server","Full audit trail","Managed delivery available"],
              },
            ].map((item, i) => (
                    ...
                    <ul className="space-y-1.5">
                      {item.points.map(p => (
                        <li key={p} className="flex items-center gap-2 text-sm">
                          <CheckCircle2 className="w-4 h-4 text-primary/60 flex-shrink-0" />{p}
                        </li>
                      ))}
                    </ul>
                  </CardContent>
      ...
      <section id="pricing" className="py-24 bg-muted/20">
        <div className="max-w-6xl mx-auto px-4">
          <motion.div {...fadeUp} className="text-center mb-12">
            ...
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-3">Choose your license. Pay once. Use forever.</h2>
            <p className="text-muted-foreground text-lg">Use your own SMTP or upgrade to managed delivery anytime. No monthly fees on the software.</p>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-6 items-start">
            {licensePlans.map((plan, i) => (
              <motion.div key={plan.name} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <Card className={cn("relative overflow-hidden h-full", plan.popular ? "border-primary shadow-2xl scale-[1.03]" : "hover:border-primary/30 transition-colors")}>
                  {plan.popular && <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-primary to-blue-500" />}
                  {plan.popular && (
                    <div className="absolute top-4 right-4">
                      <span className="text-xs font-bold bg-primary/10 text-primary px-3 py-1 rounded-full">Most Popular</span>
                    </div>
                  )}
                  <CardContent className="p-8 flex flex-col h-full">
      ...
      <section className="py-20">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <motion.div {...fadeUp} className="mb-12">
            ...
            <p className="text-muted-foreground text-lg">ZeniPost checks your DNS records automatically. Fix issues before they cost you inbox placement.</p>
          </motion.div>

          <div className="grid sm:grid-cols-3 gap-6 mb-10">
            {[
              { label: "SPF",   icon: CheckCircle,  status: "Passed",  color: "text-emerald-500", bg: "bg-emerald-50 border-emerald-200 dark:bg-emerald-500/10 dark:border-emerald-500/20", desc: "Authorises your sending server" },
              { label: "DKIM",  icon: AlertTriangle, status: "Warning", color: "text-amber-500",   bg: "bg-amber-50 border-amber-200 dark:bg-amber-500/10 dark:border-amber-500/20",   desc: "Digital signature not yet set" },
              { label: "DMARC", icon: XCircle,       status: "Missing", color: "text-red-500",     bg: "bg-red-50 border-red-200 dark:bg-red-500/10 dark:border-red-500/20",         desc: "Policy record missing" },
            ].map((r, i) => (
              <motion.div key={r.label} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <Card className={cn("border", r.bg)}>
                  <CardContent className="p-6 text-center">
                    <r.icon className={cn("w-8 h-8 mx-auto mb-3", r.color)} />
                    <h3 className="text-2xl font-extrabold mb-1">{r.label}</h3>
                    <p className={cn("text-xs font-bold uppercase tracking-wider mb-2", r.color)}>{r.status}</p>
          ...
          <motion.div {...fadeUp}>
            <p className="text-sm text-muted-foreground mb-5">Add your domain and ZeniPost walks you through fixing every record step by step.</p>
            <Link href="/register">
              <Button size="lg" className="gap-2">Check My Domain <ArrowRight className="w-4 h-4" /></Button>
            </Link>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-muted/20">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div {...fadeUp} className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-3">Fix deliverability. Stop landing in spam.</h2>
            <p className="text-muted-foreground text-lg mb-2">Increase inbox placement, reduce spam issues, and scale safely.</p>
            <p className="text-sm text-muted-foreground">Add optimized email infrastructure on top of your ZeniPost license.</p>
          </motion.div>
              ...
              <motion.div key={plan.name} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.08 }}>
                <Card className="h-full hover:shadow-lg hover:border-primary/30 transition-all">
                  <CardContent className="p-6">
                    ...
                    <p className="text-2xl font-extrabold text-primary mb-3">{plan.price}</p>
                    <div className="space-y-1.5 text-sm text-muted-foreground">
                      <p className="flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" />{plan.volume}</p>
                      <p className="flex items-center gap-1.5"><Server className="w-3.5 h-3.5" />{plan.infra}</p>
                      <p className="flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" />{plan.routing}</p>
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
          ...
          <motion.div {...fadeUp} className="text-center">
            <Button size="lg" className="gap-2">Get Managed Setup <ArrowRight className="w-5 h-5" /></Button>
          </motion.div>
        </div>
      </section>
